...
| 10.50.10.0/24 | VIP веб-интерфейса | TCP | 443 | Доступ к дашборду |
 ...
 Сегменты не пересекаются, NAT не нужен
...
- **Active Directory:** сервисы кластера обращаются к контроллеру домена по LDAPS (636) для проверки
  учетных данных пользователей; там же DNS (53).
- **SIEM:** события ИБ приложения уходят по syslog over TLS (6514), туда же - логи межсетевого экрана.
- **Служба точного времени:** узлы кластера синхронизируются по NTP (123). Единое время
...
- **Рабочие места (АРМ):** пользователи из сети службы главного механика (10.50.10.0/24) заходят на
  дашборд только по HTTPS (443), прямого доступа к брокеру, БД или узлам кластера у них нет.
