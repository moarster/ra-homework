workspace "Телеметрия карьерной техники" "Тестовое задание" {
  ...
  model {
    # Люди
    mechanic = person "Механик" "Служба главного механика, смотрит состояние парка"
    opsEngineer = person "Инженер эксплуатации" "DevOps/SRE, дежурство в рабочее время 7:00-23:00"

    # Внешние системы
    gateway = softwareSystem "Телематический шлюз" "На борту машины: опрос контроллера по Modbus, энергонезависимый буфер 1 ГБ, метка времени от ГНСС" "External,Устройство"
    ...
    signoz = softwareSystem "SigNoz" "Корпоративная платформа наблюдаемости: хранение операционных логов и метрик, дашборды, оповещения" "External,Наблюдаемость"

    tele = softwareSystem "Система телеметрии" "Прием, хранение и отображение технической телеметрии парка" {

      group "Вне Kubernetes" {
        broker = container "Брокер MQTT" "Терминация mTLS, проверка источника, ACL по топикам, постоянные сессии. Состояние: сессии и неподтвержденные сообщения на диске ВМ" "EMQX" "С состоянием,Очередь"
        tsdb = container "Хранилище временных рядов" "Измерения 12 мес., сжатие, непрерывные агрегаты. Состояние: вся телеметрия; отдельная ВМ, резервные копии и архив WAL" "PostgreSQL + TimescaleDB" "С состоянием,База данных"
      }

      group "Kubernetes" {
        ingest = container "Сервис приема" "Разбор батчей, нормализация, дедупликация по (машина, параметр, время), пакетная запись" "Go/Java"
        api = container "Сервис API" "REST и поток реального времени, аутентификация через AD, роли по группам AD" "Go/Java"
        ...
        logger = container "Сервис логирования" "Прием логов и метрик компонентов, отбор событий ИБ, маршрутизация: ИБ -> SIEM, остальное -> SigNoz. Состояние: дисковая очередь на время недоступности приемников" "OpenTelemetry Collector" "С состоянием"
      }
    }
    ...
    ingest -> broker "Подписка на топики текущих данных и догона" "MQTT over TLS, TCP 8883" "Телеметрия"
  ...
  views {
    ...
    container tele "containers" "Компоненты решения: что в Kubernetes, что вне его, компоненты с состоянием" {
      title "Контейнеры"
      include *
      autolayout lr
    }
